...
//! # Examples
//!
//! ## Creating a JWT configuration with a random RS256 keypair for testing
//!
//! ```rust
//! # #[cfg(test)]
//! # fn example() {
...
//!
//! // Generate a random RSA keypair for RS256
//! let config = JwtConfig::new_random_rs256();
//!
//! // Use the config to create and verify tokens
//! // ...
//! # }
//! ```
use std::path::Path;
...
/// Session token type enum - either a simple UUID or a JWT
#[derive(Debug, Clone, PartialEq, Eq, Hash, Serialize, Deserialize)]
#[serde(untagged)]
pub enum SessionToken {
    /// Simple UUID-based token
    Simple(String),
    /// JWT token - contains the session data within the token
    Jwt(String),
}
...
impl SessionToken {
    ...
    pub fn new(token: &str) -> Self {
        ...
        if token.chars().filter(|&c| c == '.').count() == 2 {
            ...
            SessionToken::Simple(token.to_string())
        }
    }

    /// Create a new random simple session token
    pub fn new_random() -> Self {
        SessionToken::Simple(Uuid::new_v4().to_string())
    }
    ...
    pub fn verify_jwt(&self, config: &JwtConfig) -> Result<JwtClaims, Error> {
        match self {
            ...
            SessionToken::Simple(_) => Err(Error::Session(SessionError::InvalidToken(
                "Not a JWT token".to_string(),
            ))),
        }
    }

    /// Create a new JWT session token using RS256 algorithm (legacy method)
    ...
    /// Verify a JWT session token using RS256 algorithm and return the claims (legacy method)
    ...
    pub fn into_inner(self) -> String {
        match self {
            SessionToken::Simple(token) => token,
...
impl std::fmt::Display for SessionToken {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            SessionToken::Simple(token) => write!(f, "{}", token),
